...
def main() -> None:
    ...
    ap.add_argument("--prompt", required=True, help="User prompt to run.")
    ap.add_argument(
        "--skills-dir",
        default="skills/",
    ...
    for _ in range(args.max_steps):
        ...
        for call in pending_calls:
            ...
            if name == "read_file":
                ...
                tool_calls.append({"type": "read", "path": path})
                tool_outputs.append(
                    {
                ...
                tool_calls.append(
                    {"type": "write", "path": path, "bytes": len(content)}
                )
                tool_outputs.append(
                    {
                        "type": "function_call_output",
                ...
                tool_calls.append(
                    {"type": "shell", "command": command, "exit_code": out["exit_code"]}
                )
                tool_outputs.append(
                    {
                        "type": "function_call_output",
